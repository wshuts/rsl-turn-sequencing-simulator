...
EXPECTED_56_98: list[ParsedRow] = [
    ...
    ParsedRow(88,  0, "BROKEN", "Mithrala",      "A2",  0, "BROKEN"),
    ParsedRow(89, 21, "UP",     "Fire Knight (N)",   "A1",  5, "UP"),
    ParsedRow(90,  5, "UP",     "Tomb Lord",         "A1",  2, "UP"),
    ParsedRow(91,  2, "UP",     "Coldheart",     "A1",  0, "BROKEN"),
    ParsedRow(92,  0, "BROKEN", "Mikage",      "B_A1",  0, "BROKEN"),
    ParsedRow(93,  0, "BROKEN", "Martyr",        "A1",  0, "BROKEN"),
    ParsedRow(94,  0, "BROKEN", "Mithrala",      "A1",  0, "BROKEN"),
    ParsedRow(95,  0, "BROKEN", "Tomb Lord (N)", "A3",  0, "BROKEN"),
    ParsedRow(96,  0, "BROKEN", "Mikage (N)",  "B_A1",  0, "BROKEN"),
    ParsedRow(97,  0, "BROKEN", "Coldheart (N)", "A3",  0, "BROKEN"),
    ParsedRow(98, 21, "UP",     "Fire Knight (N)",  "A2",  19, "UP"),
]


def test_cli_stdout_rows_56_98_match_expected_fields(capsys) -> None:
    """
    ...
    actual = _extract_parsed_rows_in_range(out, start=56, end=98)

    # Fail loudly if we didn't even see the rows.
    assert len(actual) == len(EXPECTED_56_98), (
        f"Expected {len(EXPECTED_56_98)} parsed rows in range 56–87, got {len(actual)}.\n"
        "This usually means row indexing drifted or the stdout format changed enough to not parse.\n"
    )

    assert actual == EXPECTED_56_98
